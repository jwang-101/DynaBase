...
for c in QQ.range_by_height(8):
    ...
    if not model_in_database_NF(F)[0]:
        label = add_function_all_NF(F, citations=['Poonen1998'], log_file=log_file)
...
for s1 in QQ.range_by_height(B):
    ...
    for s2 in QQ.range_by_height(B):
        ...
        if not model_in_database_NF(F)[0]:
            label = add_function_all_NF(F, citations=['Benedetto2009'], log_file=log_file)
...
add_function_all_NF(G, log_file=log_file)
p = 6
q = 21
G2 = DynamicalSystem([2*p*x*y-3*q*y**2,3*x**2 - p*y**2])
add_function_all_NF(G2, log_file=log_file)
